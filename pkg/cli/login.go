...
func newLoginCommand() *cobra.Command {
	var cmd = &cobra.Command{
		...
		Short:      "Log in to BoomX Docker registry",
		RunE:       login,
		Args:       cobra.MaximumNArgs(0),
	}

	cmd.Flags().Bool("token-stdin", false, "Pass login token on stdin instead of opening a browser. You can find your boomx login token at https://boomx.com/auth/token")
...
func getDisplayTokenURL(registryHost string) (string, error) {
	...
	if resp.StatusCode == http.StatusNotFound {
		return "", fmt.Errorf("%s is not the BoomX registry\nPlease log in using 'docker login'", registryHost)
	}
